...
Void button5_Click(Object^ sender, EventArgs^ e) {
	...
	if (file)										// Теперь объединяем обратно в файл a, используя последние редактируемые файлы
		simpleSortCombine1(size_demo, "b.txt", "c.txt", incount, compcount, outcount);	// Читать из b c
	else
		simpleSortCombine1(size_demo, "d.txt", "e.txt", incount, compcount, outcount);	// Читать из d e
	printStr("a.txt", row, dataGridView3); row++;
}
};
}
